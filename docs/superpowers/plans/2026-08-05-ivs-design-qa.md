...
- [ ] Criar teste que exige bloqueios por placeholder, ausência de viewport e PII em modo anônimo.
- [ ] Rodar `python3 -m unittest skills/ivs-design-qa/tests/test_static_checks.py -v` e confirmar falha por módulo ausente.
- [ ] Implementar `scan_html(path, artifact_type, data_mode)` retornando hash, blockers, concerns e métricas redigidas.
- [ ] Rodar o teste específico e a suíte.
...
- [ ] Criar teste de integração com HTML saudável e HTML com overflow.
- [ ] Rodar e confirmar falha por probe ausente.
- [ ] Instalar dependência exata com `npm install --ignore-scripts`.
- [ ] Implementar duas viewports, captura de console/pageerror, imagens quebradas, overflow e screenshots.
- [ ] Rodar integração e confirmar que o saudável passa e o overflow bloqueia.
...
- [ ] Criar testes de subprocesso para parser de `ivs-site` e Visual Layer.
- [ ] Confirmar RED.
- [ ] Implementar adaptadores com timeout, paths configuráveis e falha redigida.
- [ ] Confirmar GREEN sem tocar no original.
...
- [ ] Criar teste end-to-end para estados, exit code, JSON/HTML e SHA imutável.
- [ ] Confirmar RED.
- [ ] Implementar agregação fail-closed e relatórios sem conteúdo da página.
- [ ] Confirmar GREEN e `py_compile`.
...
- Create: `skills/ivs-design-qa/README.md`

- [ ] Criar os dois HTMLs completos, responsivos, sintéticos e sem links externos.
- [ ] Executar o gate nos dois artefatos.
- [ ] Verificar screenshots visualmente, console, overflow, hashes e redaction.
- [ ] Rodar `python3 -m unittest discover -s skills/ivs-design-qa/tests -v`, testes do Visual Layer, `git diff --check` e busca de segredos.
