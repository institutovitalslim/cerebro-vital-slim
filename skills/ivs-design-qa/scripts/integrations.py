...
def run_browser_probe(
    ...
    try:
        ...
        data = json.loads(proc.stdout or "{}")
        expected_code = 0 if data.get("ok") else 2
        ...
        return {
            "available": True,
            "ran": True,
            "ok": bool(data.get("ok")) and valid_exit,
            "returncode": proc.returncode,
            "viewports": data.get("viewports") or [],
            "blockers": blockers,
...
def run_visual_layer(
    ...
    try:
        ...
        data = json.loads(proc.stdout or "{}")
        ok = proc.returncode == 0 and bool(data.get("ok")) and unchanged
        ...
        return {
            ...
            "ok": ok,
            "returncode": proc.returncode,
            "output_html": str(data.get("output_html") or ""),
            "audit_json": str(data.get("audit_json") or ""),
            "sections_count": int(data.get("sections_count") or 0),
