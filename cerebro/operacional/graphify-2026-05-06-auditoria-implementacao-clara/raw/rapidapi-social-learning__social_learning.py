...
def normalize_x_tweets(timeline, limit=10):
    out=[]
    ...
    for t in timeline[:limit]:
        ...
        out.append({
            ...
            'screen_name': t.get('screen_name') or t.get('user',{}).get('screen_name'),
            'text': (t.get('text') or t.get('full_text') or '')[:1200],
            'favorites': t.get('favorites') or t.get('favorite_count'),
            'retweets': t.get('retweets') or t.get('retweet_count'),
...
def x_search(args):
    ...
    q=urllib.parse.urlencode({'query':args.query,'search_type':args.search_type})
    url=f'https://{host}/search.php?{q}'
    data=get_json(url, host)
    timeline=data.get('timeline') or data.get('data') or []
    items=normalize_x_tweets(timeline, args.limit)
    path=save('x-search-posts', {'query':args.query,'search_type':args.search_type,'items':items,'raw_status':data.get('status'),'next_cursor':data.get('next_cursor')})
    print(json.dumps({'ok': bool(items), 'source':'x-search', 'saved':path, 'count':len(items), 'sample':items[:5]}, ensure_ascii=False, indent=2))

def x_profile(args):
    host='twitter-api45.p.rapidapi.com'
    q=urllib.parse.urlencode({'screenname':args.screenname})
    url=f'https://{host}/timeline.php?{q}'
    data=get_json(url, host)
    timeline=data.get('timeline') or []
    items=normalize_x_tweets(timeline, args.limit)
    path=save(f'x-profile-{args.screenname}', {'screenname':args.screenname,'items':items,'next_cursor':data.get('next_cursor')})
    print(json.dumps({'ok': bool(items), 'source':'x-profile', 'saved':path, 'screenname':args.screenname, 'count':len(items), 'sample':items[:5]}, ensure_ascii=False, indent=2))
...
def x_top(args):
    ...
    timeline=data.get('timeline') or data.get('data') or []
    items=normalize_x_tweets(timeline, args.limit)
    # Fallback para busca temática, porque top_posts pode retornar vazio dependendo do país/período.
    if not items:
        class A: pass
        a=A(); a.query=args.fallback_query; a.search_type='Top'; a.limit=args.limit
        return x_search(a)
    path=save('x-top-posts', {'items':items,'raw_status':data.get('status')})
    print(json.dumps({'ok': True, 'source':'x-top', 'saved':path, 'count': len(items), 'sample': items[:5]}, ensure_ascii=False, indent=2))
...
DEFAULT_SEEDS={
 ...
 'x_clara_prioridade_1_agendamento_vendas_consultivas':['Gong_io','close','Outreach_io','patrickdang','AlexHormozi'],
 'x_clara_prioridade_2_experiencia_premium':['Hyken','willguidara','Salesforce'],
 'x_clara_prioridade_3_healthcare_jornada_paciente':['Zocdoc'],
 'x_clara_buscas_tematicas':['consultative selling objection handling','customer experience appointment booking','sales follow up no show reduction','healthcare patient scheduling experience']
}
...
def main():
    ...
    p=sub.add_parser('x-top'); p.add_argument('--type', default='Likes'); p.add_argument('--country', default='ALL'); p.add_argument('--period', default='Daily'); p.add_argument('--limit', type=int, default=10); p.add_argument('--fallback-query', default='sales objection handling OR customer experience OR consultative selling'); p.set_defaults(func=x_top)
    p=sub.add_parser('x-search'); p.add_argument('--query', required=True); p.add_argument('--search-type', default='Top'); p.add_argument('--limit', type=int, default=10); p.set_defaults(func=x_search)
    p=sub.add_parser('x-profile'); p.add_argument('--screenname', required=True); p.add_argument('--limit', type=int, default=10); p.set_defaults(func=x_profile)
