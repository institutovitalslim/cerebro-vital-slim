...
X_PROFILES=['Gong_io','close','Outreach_io','patrickdang','AlexHormozi','Hyken','Salesforce','Zocdoc']
X_SEARCHES=['consultative selling objection handling','customer experience appointment booking','sales follow up no show reduction','healthcare patient scheduling experience']
...
def slot(args):
    ...
    if slot=='instagram_manha':
        ...
        if today.timetuple().tm_yday % 2 == 0:
            profile=X_PROFILES[today.timetuple().tm_yday % len(X_PROFILES)]
            base['perfil']=profile
            base['coleta']=run(['python3',SOC,'x-profile','--screenname',profile,'--limit','5'])
        else:
            query=X_SEARCHES[today.timetuple().tm_yday % len(X_SEARCHES)]
            base['query']=query
            base['coleta']=run(['python3',SOC,'x-search','--query',query,'--search-type','Top','--limit','5'])
