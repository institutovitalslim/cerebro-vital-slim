...
- `references/replit-mcp-oauth-reconnect.md`: reconexão do MCP Replit em ambiente headless/Telegram quando OAuth expira, a porta de callback fica presa ou o navegador automatizado cai no Cloudflare; inclui limpeza de callback antigo, PTY com paste fallback e critério de aceite.
...
- `references/replit-patient-portal-recipe-builder-qa.md`: padrão para implementar e validar criação de receitas caseiras no portal paciente, incluindo ingredientes, rendimento, medidas brasileiras, cálculo estimado, CTA clicável acima da bottom nav e QA do exemplo “purê de banana-da-terra”.
...
- `references/replit-patient-portal-recipe-builder-qa.md`: QA de receita caseira no portal paciente — ingredientes da base/manual, medidas brasileiras, rendimento, cálculo por porção, reutilização e validação contra sugestões/botões interceptados pela bottom nav.
- `references/replit-mcp-oauth-headless-reconnect.md`: reconexão governada do Replit MCP em ambiente headless quando OAuth expira, incluindo limpeza da porta 59891, login com PTY, paste do callback e validação com `hermes mcp test replit`.
...
- `references/replit-barcode-scanner-preview-restart-and-clickability.md`: receita complementar para quando a URL riker retorna 404 por workflows parados e para validar/corrigir clickability do produto demo sob bottom nav; inclui probe `elementFromPoint`, reinício de preview sem alterar código, auto-scroll/spacer e gate final adicionar→conferir→salvar.
- `references/replit-tracker-preview-restart-and-full-qa.md`: checklist consolidado para Vital Slim Tracker quando o preview mostra `Run this app`/404, incluindo restart mínimo, smoke de rotas, QA de barcode, receita caseira e WhatsApp de acesso.
...
21. Em QA de scanner/barcode com bottom nav fixa, não declarar conclusão só porque o produto apareceu visualmente. Rode `document.elementFromPoint` no centro do card/produto e no CTA seguinte. Se retornar link da nav (`Progresso`, `Registrar`, etc.), o clique ainda está interceptado. `pb-44` pode não bastar; corrigir com espaço inferior real, container rolável e/ou auto-scroll ao abrir os códigos demo. Se a URL riker estiver 404 em todas as rotas, verifique primeiro se os workflows de dev estão parados e reinicie o preview sem alterar código antes de diagnosticar build quebrado. Ver `references/replit-barcode-scanner-preview-restart-and-clickability.md`.
13. Em Replit MCP, `phase: updating` não prova que um patch foi aplicado. Se o preview ainda mostra o bug ou o marcador de versão não aparece, perguntar read-only ao Replit se voltou para Plan mode antes de continuar. Quando o Agent alternar para Plan mode, pedir ao Tiaro para acionar Build e reenviar comando cirúrgico com um único critério de aceite.
...
## Receita: Portal paciente — receitas caseiras

Quando Tiaro pedir registro de preparações caseiras no Vital Slim Tracker, use `references/replit-patient-portal-recipe-builder-qa.md`. O padrão é: `Criar receita caseira` ao lado de foto/busca/código de barras; ingredientes da base ou manuais; unidades brasileiras; rendimento; cálculo por porção; reutilização futura; sempre adicionar ao draft → conferir → salvar. Pitfall obrigatório: validar sugestões de ingredientes e CTAs com `elementFromPoint`, porque a bottom nav pode interceptar cliques e navegar para `Progresso`. Não declare concluído só porque o formulário renderizou — exercite o exemplo completo no browser.
