...
- **Aviso obrigatório ao Jarvis após ingestão científica (Tiaro/Jarvis, 2026-06-27; ack Ana 2026-06-30):** sempre que pesquisa, paper ou informação médica enviada pelo Tiaro for ingerida/atualizada na memória científica, enviar pelo barramento interno `ivs-agent-message` para `jarvis`/`jarvis-ivs` com assunto começando por `BASE CIENTÍFICA ATUALIZADA — <tema>`. Corpo/evidence sanitizado deve incluir: tema, `research_id`/caminho, resumo executivo, principais achados, limitações, aplicação clínica IVS, referências/DOI/PMID quando houver e status da ingestão. Não incluir dados de pacientes, não publicar conteúdo; objetivo do Jarvis é gerar 10 posts extras em status `medical_review` para aprovação médica.
...
- Modelo preferido para conteúdo científico/médico via OpenRouter: **Kimi K3** (`moonshotai/kimi-k3`), por ordem do Tiaro em 2026-07-20, substituindo Fable 5/Opus 4.8 quando eu usaria esses modelos como validador/resposta final.
...
- **REGRA DE ROTEAMENTO DE MODELO (Tiaro, 2026-06-24; custo 2026-06-25; atualização 2026-07-20):** tarefas de LOGIN, navegação, browser e operacionais/diversas (acessar plataforma, capturar, baixar, rodar script, organizar arquivo) → usar **Codex GPT-5.5** (CLI `codex` v0.118+, ou delegar com acp_command='codex'). CONTEÚDO CIENTÍFICO/MÉDICO, ANÁLISE DE EXAMES, PESQUISAS CIENTÍFICAS e raciocínio clínico que antes usariam **Fable 5 ou Opus 4.8 via OpenRouter** → usar **Kimi K3 via OpenRouter** (`moonshotai/kimi-k3`) como VALIDADOR FINAL/juízo médico-científico, não como motor bruto. Primeiro usar memória científica, scripts locais, Gemini/Perplexity, embeddings e resumos compactos; só então chamar Kimi K3 para revisão final de nível de evidência, risco clínico/compliance e síntese executiva. Não gastar Kimi K3 em tarefa mecânica de browser/login, transcrição, OCR, chunking, lote ou leitura integral enorme. **Qualquer mudança maior no modelo padrão segue com Tiaro, mas esta substituição Fable/Opus → Kimi K3 para conteúdo está autorizada.**
...
- **OpenRouter (Kimi K3) pode esgotar:** se houver 402/quota exhausted, a análise científica deve cair para rota econômica/fallback operacional e checar `hermes auth list`, sem gastar Fable 5/Opus 4.8 salvo ordem explícita.
...
- `cerebro/operacional/graphify-2026-06-19-ana-modelo-opus-prioridade/` — RC-25 histórico: Ana = Claude Opus 4.8 prioritário. **Superado operacionalmente para conteúdo por ordem do Tiaro em 2026-07-20: usar Kimi K3 (`moonshotai/kimi-k3`) onde antes entraria Fable 5/Opus 4.8 via OpenRouter.**
...
### Modelo
- **Kimi K3 1M via OpenRouter** (`moonshotai/kimi-k3`) para conteúdo científico/médico e validação final antes atribuída a Fable 5/Opus 4.8; não usar Fable/Opus salvo ordem explícita.
