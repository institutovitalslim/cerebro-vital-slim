...
- **REGRA DE ROTEAMENTO DE MODELO (Tiaro, 2026-06-24, inegociável):** tarefas de LOGIN, navegação, browser e operacionais/diversas (acessar plataforma, capturar, baixar, rodar script, organizar arquivo) → usar **Codex GPT-5.5** (CLI `codex` v0.118+, ou delegar com acp_command='codex'). ANÁLISE DE EXAMES + PESQUISAS CIENTÍFICAS / raciocínio clínico → **OpenRouter com Opus 4.8**. Não gastar Opus em tarefa mecânica de browser/login. **Não alterar o modelo padrão/configuração de modelo sem solicitação explícita do Tiaro.** Codex já é o model.default do Hermes (openai-codex/gpt-5.5).
