# Patient Marketing Context - Instituto Vital Slim

**Documento de contexto compartilhado para todas as skills de marketing.**
**Atualizado em:** 2026-04-22
**Responsavel:** Clara (Instituto Vital Slim)

---

## 1. VISAO GERAL DA CLINICA

### Nome
Instituto Vital Slim

### Especialidade
Medicina do emagrecimento, endocrinologia, nutricao funcional e tricologia

### Diferenciais
- Protocolo medico personalizado (nao e "receita pronta")
- Acompanhamento multidisciplinar (medico + nutricionista)
- Foco em saude metabolica, nao so peso
- Tecnologia aprovada pela Anvisa
- Ambiente acolhedor e sem julgamento

---

## 2. EQUIPE

### Dra. Daniely Freitas
- **CRM-BA:** 27588
- **Especialidade:** Endocrinologia e Metabologia
- **Instagram:** @dradaniely.freitas
- **Papel:** Medica responsavel, consultas, prescricao de protocolos

### Dra. Patricia Fabrini
- **Instagram:** @patriciafabrini.dra
- **Especialidade:** Dermatologia, Tricologia
- **Papel:** Responsavel Tecnica do Centro de Tricologia Avancada
- **Credenciais:** Dermatologista SBD desde 1999, desenvolvedora da metodologia Nutroboost

### Centro de Tricologia Avancada
- Parte integrante do Instituto Vital Slim
- Especialista de referencia: Dra. Patricia Fabrini
- Servicos: tratamentos capilares avancados

### Equipe de Apoio
- Nutricionistas
- Enfermagem
- Atendimento (Clara - IA)
- Contabilidade: Medical Contabilidade

---

## 3. PUBLICO-ALVO (ICP)

### Perfil Demografico
- **Genero:** Predominantemente feminino (85%)
- **Idade:** 30-55 anos
- **Localizacao:** Salvador e regiao metropolitana, BA
- **Renda:** Classe media alta (R$ 5.000-15.000/mes)
- **Escolaridade:** Ensino superior completo

### Perfil Psicografico
- Ja tentou varias dietas e metodos sem sucesso
- Frustrada com efeito sanfona
- Busca solucao medica/sustentavel
- Valoriza saude e qualidade de vida
- Ativa nas redes sociais (Instagram)
- Leva em consideracao opiniao de outros pacientes

### Jobs to be Done (JTBD)
1. "Me ajude a perder peso de forma saudavel e sustentavel"
2. "Me de esperanca quando ja tentei de tudo"
3. "Me faca sentir cuidada e ouvida"
4. "Me de um plano claro que eu possa seguir"
5. "Me ajude a recuperar minha autoestima"

### Dores Principais
1. Cansaco de dietas que nao funcionam
2. Vergonha do espelho/roupas
3. Falta de energia no dia a dia
4. Saude comprometida (diabetes, hipertensao)
5. Fracasso repetido em metodos anteriores

### Medos
1. Efeito sanfona
2. Efeitos colaterais de medicamentos
3. Julgamento medico
4. Custo alto sem resultado
5. Nao conseguir manter o resultado

---

## 4. TRATAMENTOS E SERVICOS

### Tratamento Principal
**Protocolo de Emagrecimento Personalizado**
- Avaliacao endocrinologica completa
- Exames laboratoriais
- Plano alimentar individualizado
- Acompanhamento medico mensal
- Suporte via WhatsApp

### Outros Servicos
- Tratamento de intestino permeavel
- Reposicao hormonal
- Gestao de tireoide
- Tricologia (queda de cabelo)
- Nutricao funcional

### Precos (Confirmar com a Dra.)
- Consulta inicial: R$ [PREENCHER]
- Pacote mensal: R$ [PREENCHER]
- Pacote trimestral: R$ [PREENCHER]
- Nota: Nunca divulgar precos publicamente antes da avaliacao (politica da Clara)

---

## 5. CANAIS DE AQUISICAO

### Principal
1. Instagram (60% dos leads)
2. Google Maps/Busca (20%)
3. Indicacao de pacientes (15%)
4. Outros (5%)

### Instagram
- @institutovitalslim
- @dradaniely.freitas
- Foco: educacao + transformacao + autoridade medica

### WhatsApp (Z-API)
- Numero principal: [PREENCHER]
- Clara: atendimento automatizado com IA (ElevenLabs TTS para audio)
- Horario de atendimento: 8h-20h (BRT)
- Integracao: QuarkClinic + Omie (cadastro/financeiro)

---

## 6. OBJECOES COMUNS

### 1. Preco
**Resposta:** "O investimento varia conforme seu perfil. A avaliacao inicial e o melhor caminho para saber exatamente. Temos opcoes que cabem em diferentes realidades."

### 2. Tempo
**Resposta:** "Entendemos! Temos horarios flexiveis e tambem atendimento online para follow-ups."

### 3. Desconfianca
**Resposta:** "Completamente compreensivel. Ja ajudamos mais de 500 mulheres e entendemos sua frustracao. A primeira consulta e para voce conhecer nossa abordagem sem compromisso."

### 4. Resultado
**Resposta:** "Cada corpo responde diferente. O que garantimos e um protocolo personalizado e acompanhamento medico constante."

### 5. Concorrencia
**Resposta:** "Existem muitas opcoes mesmo. O que nos diferencia e o acompanhamento medico personalizado e nosso foco na saude metabolica, nao so no numero da balanca."

---

## 7. TOM DE VOZ

### Caracteristicas
- Empatico e acolhedor
- Especializado e confiavel
- Acessivel (sem arrogancia)
- Transformador e inspirador
- Cuidadoso e etico

### Frases que Representam
- "Entendemos sua luta"
- "Voce nao esta sozinha nessa"
- "Cada jornada e unica"
- "Sua saude em primeiro lugar"
- "Pequenos passos, grandes resultados"

### Palavras a Evitar
- "Gordo", "obeso" (como adjetivo pessoal)
- "Simples" (diminui a complexidade)
- "So" / "Apenas" (diminui o esforco)
- "Milagre" / "Revolucionario" (exageros)
- Comparacoes depreciativas

---

## 8. COMPLIANCE

### Regras Criticas
- Sempre incluir CRM da Dra. em comunicacoes oficiais
- Nunca prometer resultado especifico
- Sempre incluir disclaimer em conteudo medico
- Nunca diagnosticar em publico
- Sempre ter autorizacao para usar depoimentos/imagens

### Disclaimer Padrao
"Este conteudo tem carater informativo e educativo. Nao substitui consulta medica. Procure um medico qualificado."

---

## 9. METRICAS CHAVE

### Mensais
- Leads gerados
- Taxa de conversao (lead → consulta)
- Custo por lead
- NPS (Net Promoter Score)
- Reviews Google Maps

### Trimestrais
- Receita por paciente (LTV)
- Taxa de retencao
- Taxa de abandono
- ROI por canal

---

## 10. CONTEXTO OPERACIONAL

### Localizacao
- **Endereco:** [PREENCHER - Rua, numero, complemento]
- **Bairro:** [PREENCHER]
- **Cidade:** Salvador, BA
- **CEP:** [PREENCHER]
- **Referencia:** Proximo a [PREENCHER]

### Horario de Funcionamento
- Segunda a Sexta: 8h-18h
- Sabado: 8h-12h (confirmado)

### Contato
- **WhatsApp:** [PREENCHER - numero principal]
- **Telefone:** [PREENCHER]
- **Email:** institutovitalslim@gmail.com (confirmar)
- **Site:** www.institutovitalslim.com.br
- **Instagram:** @institutovitalslim | @dradaniely.freitas

### Sistemas e Integracoes
- **QuarkClinic:** Gestao de pacientes, agendamentos, prontuarios
- **Omie:** Faturamento, propostas, boletos
- **Z-API:** WhatsApp Business API (instancia unica)
- **Buffer:** Postagem em redes sociais (GraphQL API)
- **ElevenLabs:** TTS para respostas de audio no WhatsApp
- **Google Drive:** Armazenamento de documentos e fotos
- **Google Calendar:** Agenda de consultas

### Contas Google Workspace
- institutovitalslim@gmail.com (principal)
- medicalemagrecimento@gmail.com
- medicalcontabilidade@gmail.com

## 11. ASSETS DE MARKETING

### Fotos da Dra. (Repositorio Oficial)
- **Originais:** `/root/.openclaw/workspace/fotos_dra/originais/`
- **Avatares:** `/root/.openclaw/workspace/fotos_dra/avatares/`
- **Regra:** Sempre usar fotos reais do repositorio. Nunca gerar rosto da Dra. via IA.

### Identidade Visual
- **Cores principais:** Preto (#000000), Dourado (#d4a84b), Branco (#FFFFFF)
- **Fonte headline:** Montserrat Black
- **Badge verificacao:** Azul Twitter (#1D9BF0)

### Ferramentas de Criacao
- **Capas de carrossel:** `/root/.openclaw/workspace/skills/tweet-carrossel/scripts/make_cover.py`
- **Slides tweet:** HTML+Chromium ou Python/Pillow
- **Geracao de imagens:** NanoBanana 2 (Google) > OpenAI (fallback)

## 12. SKILLS DE MARKETING DISPONIVEIS

### Instaladas
1. `copywriting-vitalslim` — Copy para healthcare
2. `customer-research-vitalslim` — Pesquisa com pacientes
3. `social-content-vitalslim` — Conteudo social
4. `content-strategy-vitalslim` — Estrategia de conteudo
5. `whatsapp-marketing` — Fluxos WhatsApp
6. `local-seo` — SEO local
7. `medical-content` — Compliance medico

## 13. REGRAS OPERACIONAIS DE MARKETING

### Preco
- Nunca passar preco antes do paciente entender o valor
- Postura consultiva, premium, sem pressao artificial
- Oferecer avaliacao inicial como primeiro passo

### Honestidade
- Sempre dizer a verdade, mesmo quando incomoda
- Nao suavizar, nao adivinhar quando houver incerteza
- Priorizar precisao, clareza e realidade

### Imagens da Dra.
- Usar fotos reais do repositorio oficial
- Nunca gerar rosto da Dra. via IA
- Se precisar editar, apenas o fundo/contexto
- Usar clausula de consistencia facial quando usar referencia

### Referencias Cientificas
- Sempre citar estudos quando fizer afirmacoes medicas
- Formato: Autor et al. (Ano). Revista. PMID: [numero]
- Preferir meta-analises e revisoes sistematicas

---

**Nota:** Este documento deve ser atualizado mensalmente ou quando houver mudancas significativas na operacao.

**Campos pendentes de confirmacao:**
- [ ] Endereco completo
- [ ] Telefone/WhatsApp principal
- [ ] Precos de consultas e pacotes
- [x] Site oficial: www.institutovitalslim.com.br
- [ ] Email institucional
- [x] Horario de sabado: 8h-12h (confirmado)
- [ ] Bairro/regiao de atendimento
